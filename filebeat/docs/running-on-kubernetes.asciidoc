...
[IMPORTANT]
=======================================
In newer versions of Kubernetes distributions (e.g: OpenShift 4.x)  taints are used on the master nodes to affect schedulability. In this case you will need to add a toleration to your daemonset spec as per below:
+
[source,yaml]
----
spec:
